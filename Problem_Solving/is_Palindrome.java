 ...
 * Intuition
 * 
 * The first idea that comes to mind is to convert the number into string, and
 * check if the string is a palindrome, but this would require extra
 * non-constant space for creating the string which is not allowed by the
 * problem description.
 * 
 * Second idea would be reverting the number itself, and then compare the number
 * with original number, if they are the same, then the number is a palindrome.
 * However, if the reversed number is larger than int.MAX\text{int.MAX}int.MAX,
 * we will hit integer overflow problem.
 * 
 * Following the thoughts based on the second idea, to avoid the overflow issue
 * of the reverted number, what if we only revert half of the int\text{int}int
 * number? After all, the reverse of the last half of the palindrome should be
 ...
 * number "12", since 12 is the same as 12, we know that the number is a
 * palindrome.
 * 
 ...
 * 
 * Now let's think about how to revert the last half of the number. For number
 * 1221, if we do 1221 % 10, we get the last digit 1, to get the second to the
 * last digit, we need to remove the last digit from 1221, we could do so by
 * dividing it by 10, 1221 / 10 = 122. Then we can get the last digit again by
 * doing a modulus by 10, 122 % 10 = 2, and if we multiply the last digit by 10
 * and add the second last digit, 1 * 10 + 2 = 12, it gives us the reverted
 * number we want. Continuing this process would give us the reverted number
 * with more digits.
 * 
 * Now the question is, how do we know that we've reached the half of the
 * number?
 * 
 ...
 * iteration, so the time complexity is O(log⁡10(n))
 * 
